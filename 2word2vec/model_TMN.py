...
class model_TMN(object):
    def __init__(self, n_users, n_items, emb_dim, lr, lamda, text_embeddings, user_word_num, item_word_num):
        ...
        self.user_word = tf.compat.v1.placeholder(tf.int32, shape=(None, self.user_word_num))   # none, 200
        self.item_word = tf.compat.v1.placeholder(tf.int32, shape=(None, self.item_word_num))   # none, 200
        ...
        self.word_embeddings = tf.Variable(
            tf.random.normal([self.word_num, self.emb_dim], mean=0.01, stddev=0.02, dtype=tf.float32),
            name='word_embeddings')
        self.text_embeddings = tf.Variable(
            tf.convert_to_tensor(text_embeddings, dtype=tf.float32), trainable=False,
        ...
        self.u_text_attention = tf.stack([self.u_embeddings] * self.user_word_num, 1)   # 200x128
        self.u_text_attention = tf.multiply(self.u_text, self.u_text_attention)         # 200x128
        self.u_text_attention = tf.reduce_sum(self.u_text_attention, axis=2)            # 200
        ...
        self.i_text_attention = tf.nn.softmax(k * self.i_text_attention)                # 200

        self.u_text_embeddings = tf.stack([self.u_text_attention] * self.word_dim, 2)   # 200x300
