...
from dataset import Dataset, Dataset_test, Dataset_abla
...
def run(args, num_workers=1, log_interval=100, verbose=True, save_path=None):
    ...
    # initialise model
    model = model(SIDE_INFO_ITEM=300, SIDE_INFO_USER=300, emb_size=128, fc_dim=64).cuda()
   ...
   # scheduler = torch.optim.lr_scheduler.StepLR(meta_optimiser, 5000, args.lr_meta_decay)
    ...
    if not args.abla_true_data:
        dataloader_train = DataLoader(Dataset(args),
                                        batch_size=1,num_workers=args.num_workers, shuffle=True)
    else:
        dataloader_train = DataLoader(Dataset_abla(args),
                                        batch_size=1,num_workers=args.num_workers, shuffle=True)        
...
def eval(args, model, logger=None, path=None, testdataset=['new_user', 'new_item', 'new_item_user', 'old'], \
    ...
    for now_test_way in testdataset:
        ...
        dataloader_test = DataLoader(Dataset_test(args, partition=partition, test_way=now_test_way),#old, new_user, new_item, new_item_user
