...
#     print(f"{support_length}+{query_length}={all_length}")
...
#     print(true_index, false_index)
    # choose 25% true_data into support
...
#################################################################################################################################
    ...
    def __getitem__(self, item):
        ...
        # print(support_x_app, support_y_app.view(-1,1), query_x_app, query_y_app.view(-1,1))
        # print(support_y_app.view(-1,1), query_y_app.view(-1,1))
        # print('='*30)
        # exit()
        # print(support_x_app.shape, support_y_app.shape, support_y_app.view(-1,1).shape, query_y_app.shape, query_y_app.view(-1,1).shape)
...
class Metamovie_abla(Dataset):
    def __init__(self, args, partition='train', test_way=None, path=None):
        super(Metamovie_abla, self).__init__()
        self.args = args
...
# 固定了 query 和 support 所以要重写
class Metamovie_test(Dataset):
    def __init__(self, args, partition='valid', test_way=None):
        super(Metamovie_test, self).__init__()
        self.args = args
